...
| B-m7 | The review record was untracked. | **Fixed** in `86a382a`, which tracks this directory and the friction notes (`git show --stat 86a382a`). Four passes in a row had caught this row describing that commit before it existed; the hash was written in the commit after it, so the citation is past tense. Reviewer stderr files are not part of the record. |
